...
```
161.399713, 68.739907
```{r}
plot_transects = ggplot() + 
  ggRGB(kr_stack, r=1, g=2, b=3, ggLayer = TRUE, coord_equal = TRUE) +
  geom_sf(data = cherskiy, size = 4, shape = 21, fill = "grey50") +
  annotate(geom = "text", x = 161.75, y = 68.7, label = "Cherskiy", 
    fontface = "bold", color = "grey50", size = 6) +
     geom_sf(data = plot100_pt_sf, aes(fill = site),size = 4, shape = 23) +
  scale_fill_manual(values = c("tomato", "turquoise", "burlywood","thistle")) +
  scale_y_continuous(breaks = seq(67.5, 69, by = .5)) +
  scale_x_continuous(breaks = seq(160.5, 162.5, by = 1)) +
  
  annotation_north_arrow(location = "bl", which_north = "true", style = north_arrow_minimal(line_width = 1, line_col = "white", fill = "white", text_size = 10, text_face = NULL, text_family = "serif", text_col = "black")) +
...
2141125, 2141260, 2497340, 2497546  (xmin, xmax, ymin, ymax)
```{r}
coords_tr <- matrix(c(2141096, 2497300,
               2141280, 2497300,
                2141280, 2497589,
               2141096, 2497589,
               2141096, 2497300), 
...
```{r}
cn_tr1_f15_stack = stack("../data/rgb/CYN_TR1_FL015.tif")
```
...
```{r}
f15_stack_crop <- crop(cn_tr1_f15_stack, spatpoly_tr)
```
...
```{r eval=FALSE, include=FALSE}
 ggplot() + ggRGB(f15_stack_crop, r=1, g=2, b=3, ggLayer = TRUE, coord_equal = TRUE)

```

### Holding graph for UAV
```{r}
bbox_sf = st_as_sf(spatpoly_tr)
```
...
```{r fig.height=7, fig.width=6}
ggdraw(xlim = c(0, 6), ylim = c(0, 7)) +
    draw_plot(plot_transects, x = 0, y = 0, width = 3, height = 3.5) +
    draw_plot(russia_map, x = 0, y = 3.5, width = 6, height = 3.5) +      
  draw_image("../images/UAV-Veg-1Transect.PNG", scale = 0.9, x = 3, y = 0, width = 3, height = 3.5)
