...
```
```{r}
field_data = field_data %>% mutate(transect_ID =  paste(site, burn_year, transect, sep = '_'))  %>% mutate(prop_live = (live/stems*100)) %>% mutate(woody_biomass = (tree_total_biomass + shrub_total_biomass)) %>% mutate(woody_ba = (tree_total_ba + shrub_total_ba)) %>% dplyr::select(ID,transect_ID,  site:prop_live)
