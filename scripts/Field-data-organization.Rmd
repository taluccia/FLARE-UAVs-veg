...
tree_info2 = tree_info %>% mutate(ID =  paste(site, burn_year, transect, plot, sep = '_')) %>%
  dplyr::select(ID, region:plot, diameter_cm, live, radius, area_sampled_m2)
tree_info2
```
...
```{r}
trees_subset = trees %>% mutate(plot_dis = numextract(plot))  %>% dplyr::select(ID:plot, plot_dis, area_sampled_m2:total_ba) %>% rename(tree_area_sampled_m2 = area_sampled_m2, tree_radius = radius, tree_total_ba = total_ba) %>% mutate(prop_live = live/stems)
trees_subset
```
...
shrub_info2 = shrub_info %>% mutate(ID =  paste(site, burn_year, transect, plot, sep = '_')) %>%
 dplyr:: select(ID, region:plot, plot_width:basal_diameter_cm)
shrub_info2
```
