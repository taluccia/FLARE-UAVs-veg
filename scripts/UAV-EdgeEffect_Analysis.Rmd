...
# Burned vs Unburned Plus fire year

## 3. Model: GCC burn/unburned
```{r}
fit3 = lm(mean_gcc_10m ~ burn_unburn + burn_year, data = field_gcc)
```
...
```{r}
qplot(x = burn_unburn, y = res3, data = field_gcc,
xlab = "Burn v unburn group",
ylab = "Residuals",
main = "Residuals vs burn/unburn")
```
...
```
```{r}
data.frame(summary(lsmeans(fit3, pairwise ~ burn_unburn + burn_year),type = "response"))
```

#### t-test
```{r}
( fit3_t = t.test(mean_gcc_10m ~ burn_unburn + burn_year, data = field_gcc) )
```
##### Table
Difference in means
```{r}
...
#### Boxplot
```{r}
g3 = ggplot(data=field_gcc, aes(x = burn_unburn, y = mean_gcc_10m, color=burn_year) ) + # define plot axes
  geom_boxplot(outlier.shape = NA) + # Add boxplots for each group
  geom_point(aes(color = burn_unburn, shape = burn_unburn), size = 3, position = position_jitter(w = 0.15, h = 0, seed=10)) +
stat_summary(fun = mean, geom= "point", size = 4) + # Add means to the plot
  scale_shape_manual(values = c(23, 24)) +
...
## 4. Model:NDVI burn/unburned
```{r}
fit2 = lm(mean_ndvi_10m ~ burn_unburn +burn_year, data = field_gcc_ndvi)
```
...
```{r}
qplot(x = burn_unburn, y = res4, data = field_gcc_ndvi,
xlab = "Burn v unburn group",
ylab = "Residuals",
main = "Residuals vs burn/unburn")
```
...
```
```{r}
data.frame(summary(lsmeans(fit4, pairwise ~ burn_unburn),type = "response"))
```

#### t-test
```{r}
( fit4_t = t.test(mean_ndvi_10m ~ burn_unburn, data = field_gcc_ndvi) )
```
...
#### Boxplot
```{r}
g4 = ggplot(data=field_gcc_ndvi, aes(x = burn_unburn, y = mean_ndvi_10m, color=burn_unburn) ) + # define plot axes
  geom_boxplot(outlier.shape = NA) + # Add boxplots for each group
  geom_point(aes(color = burn_unburn, shape = burn_unburn), size = 3, position = position_jitter(w = 0.15, h = 0, seed=10)) +
stat_summary(fun = mean, geom= "point", size = 4) + # Add means to the plot
  scale_shape_manual(values = c(23, 24)) +
