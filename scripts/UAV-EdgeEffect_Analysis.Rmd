...
```{r}
( mod7_contrast = data.frame(summary(lsmeans(mod7, pairwise ~ burn_unburn),type = "response", infer = TRUE)$contrasts))
...
```{r}
( mod8_contrast = data.frame(summary(lsmeans(mod8, pairwise ~ burn_unburn),type = "response", infer = TRUE)$contrasts))
...
```{r}
burn_unburn_contrast = rbind(mod7_contrast, mod8_contrast)
