...
gcc_qm = gcc_qm %>% dplyr::select(plot_id, mean_gcc_25cm) %>% rename(ID = plot_id)
gcc_qm
```
...
```{r}
length(unique(gcc_1m$ID))
length(unique(gcc_3m$ID))
length(unique(gcc_qm$ID))
```


```{r}
data.frame(unique(gcc_1m$ID))
data.frame(unique(gcc_3m$ID))
data.frame(unique(gcc_qm$ID))
```

```{r}
testing = merge(gcc_1m, gcc_3m, by="ID")
testing
```
