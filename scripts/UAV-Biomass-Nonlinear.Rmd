...
fit  <- lm(live_woody_biomass_C_g_m ~ gcc + buf_res, data = field_gcc_long)
#second degree
fit2 <- lm(live_woody_biomass_C_g_m ~ poly(gcc,2,raw=TRUE) + buf_res, data = field_gcc_long)
#third degree
fit3 <- lm(live_woody_biomass_C_g_m ~ poly(gcc,3,raw=TRUE) + buf_res, data = field_gcc_long)
#fourth degree
fit4 <- lm(live_woody_biomass_C_g_m ~ poly(gcc,4,raw=TRUE) + buf_res, data = field_gcc_long)
...
```
```{r}
p <- ggplot(field_gcc_long, aes(x = gcc, y = live_woody_biomass_C_g_m, color = buf_res)) + geom_point()
