...
```{r}
(mod6_graph = data.frame(summary(lsmeans(mod6, ~ prop_live + offset(log(radius))),type = "response")))
```

```{r}

names(mod6_graph)[3:7] = c("response", "se", "df", "lcl", "ucl")
mod6_graph1 = data.frame(mod6_graph)
mod6_graph1
...
```{r}
preddat = data.frame(mod6_graph1 = tree_gcc$prop_live, radius = 5)

head(preddat)

pred = predict(mod6, newdata = preddat, se.fit = TRUE)

UL = with(pred, fit + 2*se.fit)
LL = with(pred, fit - 2*se.fit)

treesGCC = mutate(treesGCC, pred = pred$fit, 
               UL = UL, 
               LL = LL, 
               radius = 5)
```

### FIGURE  SOIL (Color, RIbbon, no Legend)
```{r}
( soil12 = ggplot(data = fire2, aes(x = prop.soil, y = seedling.ct, color = factor(fire.type2) )) + 
    geom_line(aes(y = predsoil12, color = factor(fire.type2)), linetype = 1, size = 2) + 
...
( soil13 = soil12 + 
        geom_line(aes(y=LLsoil12, color = factor(fire.type2)), linetype = 'longdash', size = 1) + 
        geom_line(aes(y=ULsoil12, color = factor(fire.type2)), linetype='longdash', size = 1)  )

( soil14 = soil13 + 
  geom_ribbon(aes(ymin=LLsoil12, ymax=ULsoil12, fill= fire.type2), linetype = 0, alpha=.25) +
