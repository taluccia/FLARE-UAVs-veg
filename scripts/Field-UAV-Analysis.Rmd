...
```{r}
mod7 = glm(mean_gcc_25cm ~ burn_unburn, data = field_gcc, family = gaussian)
```
...
```{r}
mod8 = glm(mean_ndvi_25cm ~ burn_unburn, data = field_gcc_ndvi, family = gaussian)
```
...
```{r}
mod9 = glm(mean_gcc_25cm ~ burn_year:burn_unburn, data = field_gcc, family = gaussian)
```
...
```{r}
mod10 = glm(mean_ndvi_25cm ~ burn_year:burn_unburn, data = field_gcc_ndvi, family = gaussian)
```
...
```{r}
mod11 = glm(mean_gcc_25cm ~ plot_dis, data = field_gcc, family = gaussian)
```
...
```{r}
plot_mod11 = ggplot(data = field_gcc11, aes(x = plot_dis, y = mean_gcc_25cm)) + 
    geom_point(size = 2) +  
...
```{r}
mod12 = glm(mean_ndvi_25cm ~ plot_dis, data = field_gcc_ndvi, family = gaussian)
```
...
```{r}
plot_mod12 = ggplot(data = field_gcc_ndvi12, aes(x = plot_dis, y = mean_ndvi_25cm)) + 
    geom_point(size = 2) +  
...
ggsave("../figures/2020-03-27_VI-gcc-ndvi.jpeg", plot = grid_plot_vi, width = 6, height = 7, units = c("in"), dpi=600 )
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/2020-03-27_burn-unburn-year.jpeg", plot = grid_plot_burn, width = 6, height = 6, units = c("in"), dpi=600 )
```
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/2020-03-27_distance_edge_pts.jpeg", plot = grid_plot_edge, width = 6, height = 3, units = c("in"), dpi=600 )
```
