...
## Distance from Unburned
## As it relates to vegetation cover
## GCC relates to NDVI
...
names(mod5_graph)[5:6] = c("response", "se", "df", "lcl", "ucl")
...
tree_gcc_ndvi1 = mutate(tree_gcc_ndvi, pred_mod10 = pred_mod10$fit, 
               ucl_mod10 = UL_mod10, 
               lcl_mod10 = LL_mod10)
```
...
```{r}
plot_mod10 = ggplot(data = tree_gcc_ndvi1, aes(x = total_ba, y = mean_gcc_tree)) + 
...
bquote('x axis'~(Å^2))
```{r}
plot_mod13 = ggplot(data = shrub_gcc_ndvi1, aes(x = total_ba, y = mean_gcc_shrub)) + 
