...
```{r}
mod14 = glm(mean_gcc_tree ~ live_tree_ba, data = field_gcc, family = gaussian)
```
...
```{r}
preddat_mod14 = data.frame(live_tree_ba = field_gcc$live_tree_ba)
...
```{r}
plot_mod14 = ggplot(data = field_gcc14, aes(x = live_tree_ba, y = mean_gcc_tree)) + 
  geom_point(size = 2) +
...
```{r}
mod16 = glm(mean_ndvi_tree ~ live_tree_ba, data = field_gcc_ndvi, family = gaussian)
```
...
```{r}
preddat_mod16 = data.frame(live_tree_ba = field_gcc_ndvi$live_tree_ba)
...
```{r}
plot_mod16 = ggplot(data = field_gcc_ndvi16, aes(x = live_tree_ba, y = mean_ndvi_tree)) +
  geom_point(size = 2) +
...
write.csv(model_sum5, '../outputs/model_summary/2020-06-07_vegetation-cover-gcc-ndvi.csv', row.names = F)
