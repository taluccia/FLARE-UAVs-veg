...
#### lm summary
```{r}
fit14_df = data.frame(summary(fit14)$coefficients[,], confint(fit14), extractAIC(fit14))
fit14_df
fit14_df2 = tibble::rownames_to_column(fit14_df, "coefficent")
fit14_df3 = fit14_df2 %>% 
  mutate(model = "fit14_tree_ba") %>% 
  mutate(response = "NDVI" ) %>%  
  mutate(rmse = RMSE(fit14)) %>%
  rename(estimate = Estimate, se = Std..Error, t_value = t.value,  p_value = Pr...t.., lcl = X2.5.., ucl = X97.5.., AIC = extractAIC.fit14.) %>% dplyr::select(model:response, coefficent:rmse)
fit14_df3
```
#### Data for graph
