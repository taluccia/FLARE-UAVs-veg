...
```{r}
mod10 = glm(mean_gcc_tree ~ total_ba, data = tree_gcc_ndvi, family = gaussian)
```
...
```{r}
plot_mod10 = ggplot(data = tree_gcc1, aes(x = total_ba, y = mean_gcc_tree)) + 
...
## 11. Model: GCC proportion of Live trees
...
```{r}
mod11 = glm(mean_gcc_tree ~ prop_live, data = tree_gcc, family = gaussian)
```
...
names(mod11_graph)[5:6] = c("response", "se", "df", "lcl", "ucl")
...
```{r}
preddat11 = data.frame(prop_live = tree_gcc$prop_live)
...
tree_gcc1 = mutate(tree_gcc, pred11 = pred11$fit, 
               ucl_mod11 = UL11, 
               lcl_mod11 = LL11)
```
...
```{r}
plot_mod11 = ggplot(data = tree_gcc1, aes(x = prop_live, y = mean_gcc_tree)) + 
