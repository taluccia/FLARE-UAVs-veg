...
## 15. Model: GCC 
```{r}
mod15 = glm(mean_gcc_25cm ~ burn_year:burn_unburn, data = sites_gcc, family = gaussian)
```
...
```{r}
mod16 = glm(mean_ndvi_25cm ~ burn_year:burn_unburn, data = sites_gcc_ndvi, family = gaussian)
```
...
## 19. Model: NDVIas  function of GCC
canopymod1 = glmer(canopy2 ~ mortality + burncon + (1 | id), data = biomass, family = binomial(link = "logit"))
```{r}
mod19 = glm(mean_ndvi_25cm ~ mean_gcc_25cm, data = sites_gcc_ndvi, family = gaussian)
```

### Residuals with DHARMa
```{r, include=FALSE}
mod19.res = simulateResiduals(fittedModel = mod19, n = 500)
mod19.res$scaledResiduals
mod19.res$scaledResidualsNormal
```

In the plot below we are looking for the points in the QQ plot to fall along the RED linen. In the Residuals vs.predicted plot were are looking for the lines to be relatively horizontal. All looks good. 
```{r}
plotSimulatedResiduals(simulationOutput = mod19.res)
```

```{r}
testDispersion(mod19.res)
```
...
```{r}
data.frame(summary(lsmeans(mod19, ~ mean_gcc_25cm), type = "response"))
```


```{r}
preddat_mod19 = data.frame(mean_gcc_25cm = sites_gcc_ndvi$mean_gcc_25cm)

head(preddat_mod19)

pred_mod19 = predict(mod19, newdata = preddat_mod19, se.fit = TRUE)

UL_mod19 = with(pred_mod19, fit + 2*se.fit)
LL_mod19 = with(pred_mod19, fit - 2*se.fit)

sites_gcc_ndvi19 = mutate(sites_gcc_ndvi, pred_mod19 = pred_mod19$fit, 
               ucl_mod19 = UL_mod19, 
               lcl_mod19 = LL_mod19)
```
...
plot_mod19 = ggplot(data = sites_gcc_ndvi19, aes(x = mean_gcc_25cm, y = mean_ndvi_25cm)) + 
    geom_point(size = 2) +  
    geom_line(aes(y = pred_mod19), linetype = 1, size = 2) + 
    geom_ribbon( aes(ymin = lcl_mod19, ymax = ucl_mod19, color = NULL, show.legend = FALSE), alpha = .15) +
    geom_line(aes(y= lcl_mod19), linetype = 'longdash', size = 1) + 
    geom_line(aes(y=ucl_mod19), linetype='longdash', size = 1) +
...
ggsave("../figures/2020-03-11_ndvi_gcc.jpeg", plot = plot_mod19, width = 3, height = 3, units = c("in"), dpi=600 )
```
