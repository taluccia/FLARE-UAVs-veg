...
```{r}
fit14 = lm(mean_gcc_10m ~ live_tree_ba_ha, data = field_gcc)
```
...
### 19. Model: GCC tree  BA
live_tree_ba_ha
mean_gcc_tree ~ live_tree_ba
```{r}
mod19 = glm(mean_gcc_10m ~ dead_tree_ba_ha, data = field_gcc, family = gaussian)
```
...
```{r}
mod19_df = data.frame(summary(mod19)$coefficients[,], confint(mod19), deviance(mod19), summary(mod19)$null.deviance, extractAIC(mod19))
mod19_df
mod19_df2 = tibble::rownames_to_column(mod19_df, "coefficent")
mod19_df3 = mod19_df2 %>% 
  mutate(model = "mod19_tree_ba") %>% 
  mutate(response = "GCC" ) %>%  
  rename(estimate = Estimate, se = Std..Error, t_value = t.value,  p_value = Pr...t.., lcl = X2.5.., ucl = X97.5.., deviance=deviance.mod19., null_deviance= summary.mod19..null.deviance, AIC = extractAIC.mod19.) %>% dplyr::select(model:response, coefficent:AIC)
mod19_df3
```
...
```{r}
preddat_mod19 = data.frame(dead_tree_ba_ha = field_gcc$dead_tree_ba_ha)
...
```{r}
plot_mod19 = ggplot(data = field_gcc19, aes(x = dead_tree_ba_ha, y = mean_gcc_10m)) + 
  geom_point(size = 2) +
    ...
    ylab("Mean GCC") +
    labs(x=expression(Dead~tree~BA~(m^2/ha))) +
    reg_theme +  guides(color=guide_legend(override.aes=list(fill=NA))) 
